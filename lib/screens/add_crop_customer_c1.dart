...
class _AddCropCustomerC1State extends State<AddCropCustomerC1> {
  ...
  double? quantityVal; // support fractional kg
                ...
                children: [
                  TextField(
                    ...
                    decoration: const InputDecoration(
                      labelText: 'Quantity in kg (e.g. 0.5)',
                      border: OutlineInputBorder(),
                    ),
                    onChanged: (val){
                      final parsed = double.tryParse(val.trim());
                      setStateDialog((){ quantityVal = (parsed == null || parsed <= 0) ? null : parsed; });
                    },
                  ),
  ...
  final double currentGroupAvailable = (groupAvailable ?? sumQuantityIfNoAvailable).toDouble();
  ...
  mutable['available'] = double.parse(newAvailable.toStringAsFixed(3));
...
class _CostPreview extends StatelessWidget {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          _row('Base (${_fmtQty(quantity)} kg × LKR ${unitPrice.toStringAsFixed(0)})', f(base)),
  ...
  String _fmtQty(double? q) {
    ...
    String raw = q.toStringAsFixed(3);
    raw = raw.replaceFirst(RegExp(r'\.0{1,3}'), '');
    raw = raw.replaceFirst(RegExp(r'(\.\d*?)0+'), r'$1');
